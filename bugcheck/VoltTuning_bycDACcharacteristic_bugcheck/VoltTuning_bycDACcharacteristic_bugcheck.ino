/*
 * 2021 06/11 : バグチェック用に作成．バグの内容は7日のコードで調整されたものが調整されないというもの
 * 
 * 2021 06/11 : 軸が死んでる判定を下す箇所がミスっていた．調整の際にもアンプからの出力が0になりうることを見落としていた．
 *
 *        
 *
 */
...
void setup() {
  ...
  while (TuneEnd) {
    ...
    for (int channel = 0; channel < CHANNELSIZE; ++channel) {
      ...
      if(ThreshholdTime < cnt && ADAverage[channel] == 0){
          while(1){
            digitalWrite(ledPin, LOW); 
            delay(100);
            digitalWrite(ledPin, HIGH);
